...
static void sounds_status_menu_item_class_finalize(SoundsStatusMenuItemClass *klass)
{
}

static void grab_keys(SoundsStatusMenuItemPrivate *priv)
{
...
static void ungrab_keys(SoundsStatusMenuItemPrivate *priv)
{
...
static Window hildon_window_get_active_window()
{
...
static gboolean hildon_get_dnd(Window w)
{
	if (w) {
...
static gboolean grab_zoom(SoundsStatusMenuItem *menu_item)
{
...
static double pa_vol_to_slider(int vol, int *steps, signed int steps_size)
{
	int step;
	int i;
	double step_coeff;
...
static const char *get_icon_name(int volume, gint type)
{
	if (volume == 0) {
		if (type == 1)
...
static void set_volume_icon(SoundsStatusMenuItem *menu_item, double volume)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
...
static void update_slider(SoundsStatusMenuItem *menu_item)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	double val;
...
static void sounds_status_menu_item_dispose(GObject *object)
{
...
static void sounds_status_menu_item_class_init(SoundsStatusMenuItemClass *klass)
{
#if !GLIB_CHECK_VERSION(2, 38, 0)
...
static void get_sinks(SoundsStatusMenuItemPrivate *priv)
{
...
static void create_volume_steps(SoundsStatusMenuItemPrivate *priv)
{
	gint i, n;
...
static gboolean is_running(SoundsStatusMenuItem *menu_item)
{
...
static void ext_stream_restore_read_cb(pa_context *c,
...
static void pa_ext_stream_restore_subscribe_cb(pa_context *c, void *userdata)
{
	pa_operation *o =
...
static void ext_stream_restore_test_cb(pa_context *c, uint32_t version, void *userdata)
{
	SoundsStatusMenuItem *menu_item = userdata;
	pa_operation *o;
...
static gint slider_to_pa_vol(double volume, gint *steps, gint num_steps)
{
	double vol_dbl;
...
static gboolean reset_volume_changed(gpointer user_data)
{
	SoundsStatusMenuItem *menu_item = user_data;
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
...
static double slider_volume_increase_step(SoundsStatusMenuItem *menu_item, double volume)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	gint *steps;
	gint num_steps;
...
static double slider_volume_decrease_step(SoundsStatusMenuItem *menu_item, double volume)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	gint *steps;
	gint num_steps;
...
static void draw_volume_bar(cairo_t *cr, double x, double y, double width, double height,
...
static gboolean expose_event_cb(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
	SoundsStatusMenuItem *menu_item = SOUND_STATUS_MENU_ITEM(user_data);
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
...
static gboolean parse_tuning_property(const gchar *property, gint *num_steps_out,
...
static void error_callback(pa_context *c, int success, void *userdata)
{
	if (!success)
		g_warning("VOLUME: Pulse audio failure: %s", pa_strerror(pa_context_errno(c)));
}

static void prop_sink_info_cb(pa_context *c, const pa_sink_info *i, int eol, void *userdata)
{
	SoundsStatusMenuItem *menu_item = userdata;
	SoundsStatusMenuItemPrivate *priv;
...
static void hscale_value_changed_cb(GtkRange *range, gpointer user_data)
{
...
static void parent_window_map_cb(GtkWidget *widget, SoundsStatusMenuItem *menu_item)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	pa_operation *o;
...
static void parent_window_unmap_cb(GtkWidget *widget, SoundsStatusMenuItem *menu_item)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	gboolean ret;
...
static void parent_set_cb(GtkWidget *widget, GtkWidget *old_parent,
...
static void sounds_status_menu_item_init(SoundsStatusMenuItem *menu_item)
{
	SoundsStatusMenuItemPrivate *priv = SOUND_STATUS_MENU_ITEM_PRIVATE(menu_item);
	pa_glib_mainloop *m;
	GtkWidget *hbox;
